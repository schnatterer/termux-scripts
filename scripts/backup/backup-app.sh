...
function main() {
  ...
  # For now just take the first apk. Is this enough?
  # No!! Reinstalling base.apk alone leads to app crashing with 
  ...
  # todo for sone reason tge exttacted apk is not insyallable 
...
function doRsync() {
  src="$1"
  dst="$2"
  sudo rsync --human-readable --archive ${RSYNC_EXTRA_ARG} $src $dst
}
